...
<head>
    ...
    <title>Analisis Lahan Baru - VESTNOOK</title>
    ...
    <style>
        ...
        html.dark #qr-reader { background: #111827 !important; border-color: #1e293b !important; }
...
<body class="antialiased min-h-screen flex flex-col selection:bg-[#1a73e8] selection:text-white">
    ...
    <nav class="w-full px-6 py-4 flex items-center gap-4 border-b border-gray-200 bg-white sticky top-0 z-50 shadow-sm">
        ...
        <div class="flex flex-col">
            <span class="font-semibold text-lg tracking-tight leading-tight text-[#1a73e8]">Mulai Analisis Lahan</span>
        </div>
    </nav>
    ...
    <main class="flex-grow w-full max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <!-- Selection View -->
        <div id="selection-view" class="mb-10 {{ $errors->any() ? 'hidden' : '' }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button type="button" onclick="startScanner()" class="flex flex-col items-center justify-center p-8 border-2 border-dashed border-[#1a73e8] rounded-xl hover:bg-blue-50 transition-colors group">
                    <div class="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform text-[#1a73e8]">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg>
                    </div>
                    <span class="text-lg font-semibold text-[#1a73e8]">Scan Sensor IoT (QR)</span>
                    <span class="text-sm text-gray-500 mt-1">Isi data otomatis via kamera</span>
                </button>
                <button type="button" onclick="showForm()" class="flex flex-col items-center justify-center p-8 border border-gray-200 rounded-xl hover:border-gray-300 hover:bg-gray-50 transition-colors group">
                    <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform text-gray-600">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    </div>
                    <span class="text-lg font-semibold text-gray-900">Input Manual</span>
                    <span class="text-sm text-gray-500 mt-1">Geser slider untuk isi data</span>
                </button>
            </div>
        </div>

        <!-- Scanner View -->
        <div id="scanner-view" class="hidden mb-10">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-lg font-semibold text-gray-900">Arahkan Kamera ke Layar Perangkat IoT</h2>
                <button type="button" onclick="cancelScanner()" class="text-sm text-red-600 hover:text-red-800 font-medium px-3 py-1 bg-red-50 rounded">Tutup Kamera</button>
            </div>
            <div id="reader" class="w-full overflow-hidden rounded-xl border border-gray-200 bg-black"></div>
        </div>

        <!-- Form View -->
        <div id="form-view" class="bg-white p-6 sm:p-8 rounded-xl border border-gray-100 shadow-sm {{ $errors->any() ? '' : 'hidden' }}">
            @if ($errors->any())
            ...
            <form method="POST" action="{{ route('analisis.store') }}" class="space-y-8" onsubmit="this.querySelector('button[type=submit]').disabled = true; this.querySelector('button[type=submit]').innerHTML = 'Memproses...';">
                @csrf

                <!-- Identitas -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Nama Lahan (Untuk Riwayat)</label>
                    <input type="text" name="nama_lahan" required class="input-field bg-gray-50 font-medium text-lg" placeholder="Misal: Lahan Sawah Blok A">
                </div>

                <!-- ══ Section 1: Kondisi Tanah & Zat Hara ══ -->
                <div>
                    <h3 class="section-title">Kondisi Tanah &amp; Zat Hara</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">

                        <!-- Tipe Tanah (dropdown tetap) -->
                        <div class="col-span-1 sm:col-span-2">
                            ...
                            <select name="Soil_Type" class="input-field" required>
                                ...
                                <option value="Sandy">Sandy (Berpasir)</option>
                                <option value="Loamy">Loamy (Gembur)</option>
                                <option value="Silt">Silt (Lanau)</option>
                                <option value="Peaty">Peaty (Gambut)</option>
                                <option value="Chalky">Chalky (Kapur)</option>
                            </select>
                        </div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Soil_pH">6.5</span>
                                        <span class="slider-desc-pill" id="pill-Soil_pH" style="background:#d1fae5;color:#065f46">Netral</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Soil_pH" min="3.5" max="9.5" step="0.1" value="6.5" oninput="updateSlider('Soil_pH',this.value)">
                                <input type="hidden" name="Soil_pH" id="input-Soil_pH" value="6.5" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Soil_Moisture">30</span>
                                        <span class="slider-desc-pill" id="pill-Soil_Moisture" style="background:#fef08a;color:#92400e">Kering</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Soil_Moisture" min="0" max="100" step="1" value="30" oninput="updateSlider('Soil_Moisture',this.value)">
                                <input type="hidden" name="Soil_Moisture" id="input-Soil_Moisture" value="30" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Organic_Carbon">1.5</span>
                                        <span class="slider-desc-pill" id="pill-Organic_Carbon" style="background:#fed7aa;color:#c2410c">Rendah</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Organic_Carbon" min="0" max="6" step="0.1" value="1.5" oninput="updateSlider('Organic_Carbon',this.value)">
                                <input type="hidden" name="Organic_Carbon" id="input-Organic_Carbon" value="1.5" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Electrical_Conductivity">1.0</span>
                                        <span class="slider-desc-pill" id="pill-Electrical_Conductivity" style="background:#d1fae5;color:#065f46">Normal</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Electrical_Conductivity" min="0" max="6" step="0.1" value="1.0" oninput="updateSlider('Electrical_Conductivity',this.value)">
                                <input type="hidden" name="Electrical_Conductivity" id="input-Electrical_Conductivity" value="1.0" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Nitrogen_Level">50</span>
                                        <span class="slider-desc-pill" id="pill-Nitrogen_Level" style="background:#d1fae5;color:#065f46">Cukup</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Nitrogen_Level" min="0" max="200" step="1" value="50" oninput="updateSlider('Nitrogen_Level',this.value)">
                                <input type="hidden" name="Nitrogen_Level" id="input-Nitrogen_Level" value="50" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Phosphorus_Level">30</span>
                                        <span class="slider-desc-pill" id="pill-Phosphorus_Level" style="background:#d1fae5;color:#065f46">Cukup</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Phosphorus_Level" min="0" max="150" step="1" value="30" oninput="updateSlider('Phosphorus_Level',this.value)">
                                <input type="hidden" name="Phosphorus_Level" id="input-Phosphorus_Level" value="30" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Potassium_Level">100</span>
                                        <span class="slider-desc-pill" id="pill-Potassium_Level" style="background:#d1fae5;color:#065f46">Cukup</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Potassium_Level" min="0" max="500" step="1" value="100" oninput="updateSlider('Potassium_Level',this.value)">
                                <input type="hidden" name="Potassium_Level" id="input-Potassium_Level" value="100" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Temperature">25</span>
                                        <span class="slider-desc-pill" id="pill-Temperature" style="background:#fef3c7;color:#92400e">Hangat</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Temperature" min="5" max="50" step="0.5" value="25" oninput="updateSlider('Temperature',this.value)">
                                <input type="hidden" name="Temperature" id="input-Temperature" value="25" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Humidity">55</span>
                                        <span class="slider-desc-pill" id="pill-Humidity" style="background:#d1fae5;color:#065f46">Sedang</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Humidity" min="0" max="100" step="1" value="55" oninput="updateSlider('Humidity',this.value)">
                                <input type="hidden" name="Humidity" id="input-Humidity" value="55" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Rainfall">100</span>
                                        <span class="slider-desc-pill" id="pill-Rainfall" style="background:#d1fae5;color:#065f46">Sedang</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Rainfall" min="0" max="500" step="1" value="100" oninput="updateSlider('Rainfall',this.value)">
                                <input type="hidden" name="Rainfall" id="input-Rainfall" value="100" required>
                                <div class="slider-ticks"><span class="slider-tick-label">0 Kering</span><span class="slider-tick-label">500 Lebat</span></div>
                            </div>
                        </div>

                        <!-- Region (dropdown tetap) -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Region</label>
                            <select name="Region" class="input-field" required>
                                <option value="">Pilih Region</option>
                                <option value="North">Utara (North)</option>
                                <option value="South">Selatan (South)</option>
                                <option value="East">Timur (East)</option>
                                <option value="West">Barat (West)</option>
                                <option value="Central">Tengah (Central)</option>
                            </select>
                        </div>
                    </div>
                </div>

                <!-- ══ Section 3: Informasi Tanaman & Panen ══ -->
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="combobox-wrap" id="wrap-Crop_Type">
                                <input type="text" class="input-field combobox-input" placeholder="Cari atau ketik tanaman..." id="display-Crop_Type" autocomplete="off" oninput="filterCombo('Crop_Type')" onfocus="openCombo('Crop_Type')" onblur="closeCombo('Crop_Type')">
                                <input type="hidden" name="Crop_Type" id="val-Crop_Type" required>
                        ...
                        <div>
                            ...
                            <select name="Crop_Growth_Stage" class="input-field" required>
                                ...
                                <optgroup label="Awal Tanam">
                                    <option value="Germination">Perkecambahan - baru mulai tumbuh dari benih</option>
                                    <option value="Seedling">Bibit / Persemaian - tanaman masih kecil</option>
                                </optgroup>
                                <optgroup label="Masa Tumbuh">
                                    <option value="Vegetative">Pertumbuhan Daun &amp; Batang - tanaman aktif berkembang</option>
                                    <option value="Tillering">Anakan - mulai membentuk rumpun (padi/jagung)</option>
                                </optgroup>
                                <optgroup label="Pembungaan &amp; Buah">
                                    <option value="Flowering">Berbunga - muncul bunga</option>
                                    <option value="Fruiting">Pembentukan Buah - buah mulai terbentuk</option>
                                    <option value="Ripening">Pematangan - buah/biji sedang matang</option>
                                </optgroup>
                                <optgroup label="Akhir Siklus">
                                    <option value="Harvest">Siap Panen - waktu memanen</option>
                                    <option value="Post-Harvest">Pasca Panen - setelah dipanen, lahan istirahat</option>
                                </optgroup>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="Season" class="input-field" required>
                                ...
                                <optgroup label="Musim di Indonesia">
                                    <option value="Kharif">Musim Hujan - curah hujan tinggi (biasanya Apr-Sep)</option>
                                    <option value="Rabi">Musim Kemarau - curah hujan rendah/kering (Okt-Mar)</option>
                                    <option value="Zaid">Musim Pancaroba / Peralihan - antara hujan dan kemarau</option>
                                </optgroup>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="Irrigation_Type" class="input-field" required>
                                ...
                                <optgroup label="Aliran Air">
                                    <option value="Canal">Saluran Irigasi / Got - air mengalir dari sungai/saluran</option>
                                    <option value="Flood">Leb / Banjiran - lahan digenangi air (sawah)</option>
                                    <option value="Furrow">Alur Parit - air dialirkan melalui parit kecil di antara tanaman</option>
                                </optgroup>
                                <optgroup label="Semprot &amp; Tetes">
                                    <option value="Sprinkler">Sprinkler / Penyiram Putar - disemprot dari atas</option>
                                    <option value="Drip">Tetes (Drip) - selang tetes langsung ke akar</option>
                                </optgroup>
                                <optgroup label="Tidak Ada Irigasi">
                                    <option value="Rainfed">Tadah Hujan - hanya mengandalkan curah hujan</option>
                                    <option value="Manual">Siram Manual - disiram dengan ember/pompa tangan</option>
                                </optgroup>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="combobox-wrap" id="wrap-Previous_Crop">
                                <input type="text" class="input-field combobox-input" placeholder="Cari atau ketik tanaman sebelumnya..." id="display-Previous_Crop" autocomplete="off" oninput="filterCombo('Previous_Crop')" onfocus="openCombo('Previous_Crop')" onblur="closeCombo('Previous_Crop')">
                                <input type="hidden" name="Previous_Crop" id="val-Previous_Crop" required>
                        ...
                        <div>
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Fertilizer_Used_Last_Season">100</span>
                                        <span class="slider-desc-pill" id="pill-Fertilizer_Used_Last_Season" style="background:#d1fae5;color:#065f46">Sedang</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Fertilizer_Used_Last_Season" min="0" max="600" step="5" value="100" oninput="updateSlider('Fertilizer_Used_Last_Season',this.value)">
                                <input type="hidden" name="Fertilizer_Used_Last_Season" id="input-Fertilizer_Used_Last_Season" value="100" required>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="slider-widget">
                                <div class="slider-header">
                                    ...
                                    <div class="slider-value-badge">
                                        <span class="slider-value-num" id="disp-Yield_Last_Season">3.0</span>
                                        <span class="slider-desc-pill" id="pill-Yield_Last_Season" style="background:#fef3c7;color:#92400e">Rata-rata</span>
                                    </div>
                                </div>
                                <input type="range" class="styled-range" id="range-Yield_Last_Season" min="0" max="15" step="0.1" value="3.0" oninput="updateSlider('Yield_Last_Season',this.value)">
                                <input type="hidden" name="Yield_Last_Season" id="input-Yield_Last_Season" value="3.0" required>
    ...
    <script>
        ...
        function updateSlider(fieldId, val) {
            ...
            // 4. Update slider track fill (color up to thumb, gray after)
    ...
    <script>
        ...
        function startScanner() {
            ...
            ).catch(() => { alert("Tidak dapat mengakses kamera."); cancelScanner(); });
        }
        ...
        function onScanSuccess(decodedText) {
            ...
            try {
                ...
                const fields = ['Soil_pH','Soil_Moisture','Organic_Carbon','Electrical_Conductivity',
                                'Nitrogen_Level','Phosphorus_Level','Potassium_Level',
                                'Temperature','Humidity','Rainfall'];
                fields.forEach(field => {
                    if (data[field] !== undefined) {
                        const el = document.getElementById('range-' + field);
                        if (el) { el.value = data[field]; updateSlider(field, data[field]); }
                    }
                });
            } catch (e) {
                alert("Format QR Code tidak valid. Form tetap terbuka untuk input manual.");
            }
            document.getElementById('scanner-view').classList.add('hidden');
            document.getElementById('selection-view').classList.remove('hidden');
            document.getElementById('form-view').classList.remove('hidden');
            window.scrollTo({ top: document.getElementById('form-view').offsetTop, behavior: 'smooth' });
        }
